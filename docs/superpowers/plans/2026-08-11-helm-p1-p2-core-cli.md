...
test('lastActivityMs 取該專案所有 session 的最大值', () => {
  const out = groupIntoProjects(
    [sess({ sessionId: 'a', updatedAt: 100 }), sess({ sessionId: 'b', updatedAt: 900 })],
    deps,
  )
  assert.equal(out[0]?.lastActivityMs, 900)
})
...
test('專案內的 session 依 updatedAt 由新到舊排序', () => {
  const out = groupIntoProjects(
    [sess({ sessionId: 'old', updatedAt: 1 }), sess({ sessionId: 'new', updatedAt: 9 })],
    deps,
  )
