...
const res = (projects: ProjectView[], invalid = 0): Board => ({ projects, invalid, prefsHealth: 'ok' as const, adapterFailures: [] })
...
test('偏好檔毀損時在輸出中明講，並告知原檔沒有被丟掉', () => {
  ...
  const out = renderTable({ projects: [proj({})], invalid: 0, prefsHealth: 'quarantined' as const, adapterFailures: [] }, opts)
